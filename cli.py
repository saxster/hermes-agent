...
from hermes_cli.worktree import setup_worktree, cleanup_worktree, prune_stale_worktrees, _git_repo_root
...
from hermes_cli.cmd_handlers.handlers import CLICommandHandlersMixin

import hermes_cli.cmd_handlers.handlers as _hcv
_hcv._cprint = _cprint
_hcv._rich_text_from_ansi = _rich_text_from_ansi
_hcv._accent_hex = _accent_hex
_hcv.ChatConsole = ChatConsole
_hcv.AIAgent = AIAgent
_hcv.build_plan_path = build_plan_path
_hcv.build_skill_invocation_message = build_skill_invocation_message
_hcv._RST = getattr(sys.modules[__name__], "_RST", "")
_hcv._DIM = getattr(sys.modules[__name__], "_DIM", "")
_hcv._GOLD = getattr(sys.modules[__name__], "_GOLD", "")

class HermesCLI(CLICommandHandlersMixin):
    """
    ...
    def _enable_voice_mode(self):
        ...
        try:
            from hermes_cli.config import load_config
            _raw_ptt = load_config().voice.record_key
    ...
    def _show_voice_status(self):
        ...
        _raw_key = load_config().voice.record_key
...
def main(
    ...
    if not list_tools and not list_toolsets:
        ...
        if use_worktree:
            ...
            if _repo:
                prune_stale_worktrees(_repo)
            wt_info = setup_worktree()
            if wt_info:
                os.environ["TERMINAL_CWD"] = wt_info["path"]
                atexit.register(cleanup_worktree, wt_info)
            else:
