"""Shared auxiliary client router — base module (F-C3 step 1).

The auxiliary client used to live in ``agent/auxiliary_client.py`` as a
single ~1,930-line module. F-C3 peels it into a package under
``agent/auxiliary/`` so vision, provider adapters, and resolution logic
can move out over subsequent commits. Step 1 only relocates the file —
no API change. ``agent/auxiliary_client.py`` becomes a thin shim that
re-exports everything from ``agent.auxiliary``.
...
# Provider adapter classes (CodexAuxiliaryClient, AnthropicAuxiliaryClient,
# their async wrappers, and the sync chat/completions shims behind them)
# extracted to ``agent/auxiliary/providers.py`` (F-C3 step 3). They are
# imported back here so base-module code that constructs Codex/Anthropic
# wrappers (``_try_codex`` → CodexAuxiliaryClient, ``_try_anthropic`` →
# AnthropicAuxiliaryClient, ``_to_async_client`` → the Async*Client
# wrappers) keeps using the unqualified names.
from agent.auxiliary.providers import (  # noqa: E402
...
# Vision-specific resolution (auto-selection order, backend availability,
# resolve_vision_provider_client, get_vision_auxiliary_client) extracted to
# ``agent/auxiliary/vision.py`` (F-C3 step 2). The package ``__init__``
# re-exports everything from vision.py so ``from agent.auxiliary import
# resolve_vision_provider_client`` (and every other vision helper) keeps
# working. The legacy ``agent/auxiliary_client.py`` shim also re-exports
# private names like ``_normalize_vision_provider`` and
# ``_resolve_strict_vision_backend`` so tests that import them keep
# resolving.
