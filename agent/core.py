...
from agent.client_manager import LLMClientManager
...
class AIAgent:
    ...
    def __init__(
        ...
        self.client_manager = LLMClientManager(self)
        ...
        if self.api_mode == "anthropic_messages":
            ...
            try:
                self.client = self.client_manager.create_client(client_kwargs, reason="agent_init", shared=True)
                if not self.quiet_mode:
        ...
        try:
            ...
            print(f"DEBUG: Exception in _load_agent_config: {e}")
            from hermes_cli.config_schema import HermesConfig
            _agent_cfg = HermesConfig()

        print(f"DEBUG: _agent_cfg is type {type(_agent_cfg)}, value {_agent_cfg}")
        ...
        _council_cfg = _agent_cfg.council
        ...
        try:
            guardrail_config = _agent_cfg.guardrails
            if guardrail_config:
        ...
        if not skip_memory:
            try:
                graph_config = _agent_cfg.context_graph
        ...
        if self._session_db:
            try:
                kn_config = _agent_cfg.knowledge
        ...
        try:
            skills_config = _agent_cfg.skills
        ...
        _agent_section = _agent_cfg.agent
        self._tool_use_enforcement = _agent_section.tool_use_enforcement

        # Initialize context compressor for automatic context management
        # Compresses conversation when approaching model's context limit
        # Configuration via config.yaml (compression section)
        _compression_cfg = _agent_cfg.compression
        ...
        if _config_context_length is None:
            _custom_providers = _agent_cfg.custom_providers
    ...
    def _close_openai_client(self, client: Any, *, reason: str, shared: bool) -> None:
        ...
        force_closed = self.client_manager._force_close_tcp_sockets(client)
        try:
            client.close()
            logger.info(
    ...
    def _replace_primary_openai_client(self, *, reason: str) -> bool:
        with self._openai_client_lock():
            ...
            try:
                new_client = self.client_manager.create_client(self._client_kwargs, reason=reason, shared=True)
            except Exception as exc:
                logger.warning(
        ...
        self.client_manager.close_client(old_client, reason=f"replace:{reason}", shared=True)
        return True
    ...
    def _ensure_primary_openai_client(self, *, reason: str) -> Any:
        ...
        if not self.client_manager.recreate_client(reason=f"recreate_closed:{reason}"):
    ...
    def _cleanup_dead_connections(self) -> bool:
        ...
        try:
            ...
            if dead_count > 0:
                ...
                self.client_manager.recreate_client(reason="dead_connection_cleanup")
                return True
        except Exception as exc:
    ...
    def _create_request_openai_client(self, *, reason: str) -> Any:
        ...
        primary_client = self.client_manager.get_client(reason=reason)
        if isinstance(primary_client, Mock):
            return primary_client
        with self._openai_client_lock():
            request_kwargs = dict(self._client_kwargs)
        return self.client_manager.create_client(request_kwargs, reason=reason, shared=False)

    def _close_request_openai_client(self, client: Any, *, reason: str) -> None:
        self.client_manager.close_client(client, reason=reason, shared=False)

    def _run_codex_stream(self, api_kwargs: dict, client: Any = None, on_first_delta: callable = None):
        """Execute one streaming Responses API request and return the final response."""
        import httpx as _httpx

        active_client = client or self.client_manager.get_client(reason="codex_stream_direct")
    ...
    def _run_codex_create_stream_fallback(self, api_kwargs: dict, client: Any = None):
        ...
        active_client = client or self.client_manager.get_client(reason="codex_create_stream_fallback")
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        if not self.client_manager.recreate_client(reason="codex_credential_refresh"):
            return False
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        if not self.client_manager.recreate_client(reason="nous_credential_refresh"):
            return False
    ...
    def _swap_credential(self, entry) -> None:
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        self.client_manager.apply_client_headers_for_base_url(self.base_url)
        self.client_manager.recreate_client(reason="credential_rotation")
    ...
    def _anthropic_messages_create(self, api_kwargs: dict):
        if self.api_mode == "anthropic_messages":
            self.client_manager.try_refresh_anthropic_credentials()
        ...
        def _call():
            try:
                if self.api_mode == "codex_responses":
                    request_client_holder["client"] = self.client_manager.create_request_client(reason="codex_stream_request")
                    ...
                    request_client_holder["client"] = self.client_manager.create_request_client(reason="chat_completion_request")
                ...
                if request_client is not None:
                    self.client_manager.close_request_client(request_client, reason="request_complete")
            ...
            if self._interrupt_requested:
                ...
                try:
                    if self.api_mode == "anthropic_messages":
                        ...
                        request_client = request_client_holder.get("client")
                        if request_client is not None:
                            self.client_manager.close_request_client(request_client, reason="interrupt_abort")
                except Exception:
                    pass
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            request_client_holder["client"] = self.client_manager.create_request_client(
        ...
        def _call():
            ...
            try:
                        ...
                        if _is_timeout or _is_conn_err or _is_sse_conn_err:
                            ...
                            if _stream_attempt < _max_stream_retries:
                                ...
                                if stale is not None:
                                    self.client_manager.close_request_client(
                ...
                request_client = request_client_holder.get("client")
                if request_client is not None:
                    self.client_manager.close_request_client(request_client, reason="stream_request_complete")
        ...
        while t.is_alive():
            ...
            if time.time() - last_chunk_time["t"] > _stream_stale_timeout:
                ...
                try:
                    ...
                    if rc is not None:
                        self.client_manager.close_request_client(rc, reason="stale_stream_kill")
                except Exception:
                    pass
                # Rebuild the primary client too — its connection pool
                # may hold dead sockets from the same provider outage.
                try:
                    self.client_manager.recreate_client(reason="stale_stream_pool_cleanup")
                except Exception:
                    pass
            ...
            if self._interrupt_requested:
                try:
                    if self.api_mode == "anthropic_messages":
                        ...
                        request_client = request_client_holder.get("client")
                        if request_client is not None:
                            self.client_manager.close_request_client(request_client, reason="stream_interrupt_abort")
                except Exception:
                    pass
    ...
    def flush_memories(self, messages: list = None, min_turns: int = None):
        ...
        try:
            ...
            if not _aux_available and self.api_mode == "codex_responses":
                ...
                response = self.client_manager.get_client(reason="flush_memories").chat.completions.create(
        ...
        try:
            ...
            if self.api_mode == "codex_responses":
                ...
                if self.api_mode == "anthropic_messages":
                    ...
                    summary_response = self.client_manager.get_client(reason="iteration_limit_summary").chat.completions.create(**summary_kwargs)
            ...
            if final_response:
                ...
                if self.api_mode == "codex_responses":
                    ...
                    summary_response = self.client_manager.get_client(reason="iteration_limit_summary_retry").chat.completions.create(**summary_kwargs)
    ...
    def run_conversation(
        ...
        if self.api_mode != "anthropic_messages":
            try:
                if self.client_manager.cleanup_dead_connections():
                    self._emit_status(
        ...
        # F-005: iteration_budget is the authoritative counter. `api_call_count`
        # is preserved only as a belt-and-suspenders hard cap — callers that
        # supply `max_iterations` but not an explicit budget will see
        # iteration_budget.max_total == max_iterations, so the two conditions
        # converge. When they diverge (e.g., tools that self-refund via
        # `iteration_budget.refund(...)`), the budget is what stops the loop
        # first. Delegation trees rely on this: each child inherits a budget
        # slice. Do NOT add new work counters here without updating budget.py.
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if (
                        ...
                        if self.client_manager.try_refresh_codex_credentials(force=True):
                    ...
                    if (
                        ...
                        if self.client_manager.try_refresh_nous_credentials(force=True):
                    ...
                    if (
                        ...
                        from agent.anthropic_adapter import _is_oauth_token
                        if self.client_manager.try_refresh_anthropic_credentials():
                ...
                if assistant_message.tool_calls:
                    ...
                    if invalid_tool_calls:
                        ...
                        if self._invalid_tool_retries >= 3:
                            ...
                            self._invalid_tool_retries = 0
                            # F-006: strip invalid tool_calls from any assistant
                            # messages before persisting, so a session resume
                            # doesn't replay the same bad call in a cross-session
                            # loop. We keep the textual content (model thinking)
                            # but remove the malformed tool_calls array.
                            valid_names = self.valid_tool_names
                            scrubbed_count = 0
                            for msg in messages:
                                if not isinstance(msg, dict):
                                    continue
                            ...
                            if scrubbed_count:
                                self._vprint(
                                    f"{self.log_prefix}🧹 Scrubbed {scrubbed_count} invalid tool_call(s) "
                                    "from messages before persistence (F-006)",
                                    force=True,
                                )
                            self._persist_session(messages, conversation_history)
                            return {
